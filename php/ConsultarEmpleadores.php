<?php
  ...
  if(mysqli_num_rows($asd)>1){
    while ($row = mysqli_fetch_array($asd)) {
      ...
      echo '<a onclick="return MostrarModalConsultar('.$row['id_Usuario'].');" data-toggle="modal" data-target="#Mod" class="thumbnail">';
      if (file_exists($archivo)){
      echo '<img src="'.$archivo.'"  style="width:180px;height:100%\px;"><p>'.$row['Nom'].' '.$row['Ap'].' '.$row['Am'].'</p></a>';
    }else{
    ...
    echo '<a onclick="return MostrarModalConsultar('.$row['id_Usuario'].');" data-toggle="modal" data-target="#Mod" class="thumbnail">';
    if (file_exists($archivo)){
      echo '<img src="'.$archivo.'"  width="180px" height="100px"><p>'.$row['Nom'].' '.$row['Ap'].' '.$row['Am'].'</p></a>';
    }else{
         echo '<img src="'.$ruta.'user.png"  width="180px" height="100px"><p>'.$row['Nom'].' '.$row['Ap'].' '.$row['Am'].'</p></a>';
    }
    echo '</div>';
