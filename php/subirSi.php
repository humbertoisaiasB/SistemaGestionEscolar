<?php
	...
	elseif ($row1['Tipo']=='Director') {
		...
		echo "es : ".$_POST['tipo'];
	}elseif ($row1['Tipo']=='SubDirector') {
		$asd = mysqli_query($con,"select pe.curpAdmi,u.Nom,u.Ap,u.Am,u.tipo from usuarios as u inner join subDirector as sd on(sd.id_usuario=u.id_usuario) where u.id_usuario=".$_SESSION['id']."");
		$row = mysqli_fetch_array($asd);
		$nombre_temporal = $_FILES['archivo']['tmp_name'];
		$nombre = $_FILES['archivo']['name'];
		$nombreF = $_SESSION['nombreD'].".pdf";
		...
		move_uploaded_file($nombre_temporal,'documentos/subDirector/'.$row['curpSubDirector'].'/'.$nombreF); 
		echo "es : ".$_POST['tipo'];
	}
